...
function initAudio() {
    // Fix for Incorrect pitch on mobile devices (https://github.com/rism-digital/midi-player/issues/21)
...
function startAudio() {
    //re-set onaudioprocess as unset when stopped
...
function completeConversion(status) {
    ...
    // Not a pause
    if (_EM_signalStop != 2) {
        // Fix last note played stays highlighted
        setTimeout(function() {currentPlayer.stop();}, 1000);
    }
}
...
function onAudioProcess(audioProcessingEvent) {
    ...
    if (outputBuffer.copyToChannel !== undefined) {
        ...
        // copyToChannel is available on all browsers but IE
        ...
        // Other browsers (only IE) -> about 20 - 70% slower than decoding
...
class MidiPlayerClass {
    ...
    constructor(htmlPlayerElem, options, msv) {
        // creates player of name htmlPlayerElem
        // replaced $ by jQuery() for compatibility old WordPress version
        ...
        jQuery("#midiPlayer_div_"+htmlPlayerElem).append('<div id="midiPlayer_playingTime_'+htmlPlayerElem+'" class="midiPlayer_playingTime">0:00</div>')
            ...
            .append('<div id="midiPlayer_controllers_'+htmlPlayerElem+'" class="midiPlayer_controllers"><a class="icon play" id="midiPlayer_play_'+htmlPlayerElem+'" onclick="'+htmlPlayerElem+'.getPlayer().clickPlay()"></a><a class="icon pause" id="midiPlayer_pause_'+htmlPlayerElem+'" onclick="'+htmlPlayerElem+'.getPlayer().pause()"></a><a class="icon stop" id="midiPlayer_stop_'+htmlPlayerElem+'" onclick="'+htmlPlayerElem+'.getPlayer().stop()"></a></div>');
        ...
        this.midiPlayer_bar.addEventListener('touchstart', function (e) {
                e.preventDefault();
                if (midiPlayer_totalSamples == 0) return;
                var epageX = e.changedTouches[0].pageX;
                pageDragStart = epageX;
                var rect = e.target.getBoundingClientRect();
                barDragStart = epageX - rect.left;
                updateDragging(epageX);
        });
        ...
        this.midiPlayer_bar.addEventListener('touchmove', function (e) {
                e.preventDefault();
                if (pageDragStart != 0) {
                    that.pause();
                    updateDragging(e.changedTouches[0].pageX);
                }
        });
        ...
        this.midiPlayer_bar.addEventListener('touchend', function (e) {
                e.preventDefault();
                if (pageDragStart == 0) return;
                if (midiPlayer_totalSamples == 0) return;
                pageDragStart = 0;
        ...
        function updateDragging(pageX) {
            ...
            if (posX >= 0 && posX <= currentPlayer.midiPlayer_bar.clientWidth) {
                var percent = posX / currentPlayer.midiPlayer_bar.clientWidth;
    ...
    clickPlay()
    {
        //If AudioContext is already created on page load, we have to call resume() after the user interacted with the page as the autoplay policy is disabled on Chrome
        if (midiPlayer_isAudioInit) audioCtx.resume();
        this.play();
    }
    
    play() {
        // If another player is playing we stop it, or if we navigate inside another reader
        if (currentPlayer &&
            (currentPlayer != this
            || this.getMsv() != curMsv))
        {
    ...
    stop() {
        ...
        //Unset onaudioprocess as continuously called (on FireFox)
